...
$stmt_update = $con->prepare("UPDATE events SET pelihat = pelihat + 1 WHERE id_event = ?");
$stmt_update->bind_param("i", $id_event);
$stmt_update->execute();
$stmt_update->close();

// --- 3. Ambil Data Event ---
$stmt_select = $con->prepare("SELECT judul, gambar, isi, tanggal, kategori, pelihat, crop_y FROM events WHERE id_event = ? AND status = 'published'");
$stmt_select->bind_param("i", $id_event);
$stmt_select->execute();
...
if ($result->num_rows === 1) {
    ...
    $judul = htmlspecialchars($event_data['judul']);
    $isi_html = $event_data['isi'];
    ...
    $tanggal_obj = date_create($event_data['tanggal']);
    $tanggal = date_format($tanggal_obj, 'd') . ' ' . 
               $bulan_indonesia[date_format($tanggal_obj, 'n')] . ' ' . 
               date_format($tanggal_obj, 'Y');
    
    $kategori = htmlspecialchars($event_data['kategori']);
    $pelihat = number_format($event_data['pelihat']);
    $gambar_url = !empty($event_data['gambar']) ? 'adminpanel/gambar/' . htmlspecialchars($event_data['gambar']) : '';
    $crop_position = $event_data['crop_y'] . '%';
} else {
    $error_message = "Event tidak ditemukan atau belum dipublikasikan.";
    goto display_error;
}
$stmt_select->close();

// --- 4. Ambil Event Terkait (6 event berdasarkan kategori yang sama) ---
$stmt_related = $con->prepare("SELECT id_event, judul, tanggal, kategori, pelihat, gambar, crop_y 
                                FROM events 
                                WHERE kategori = ? AND id_event != ? AND status = 'published'
                                ORDER BY tanggal DESC 
                                LIMIT 6");
$stmt_related->bind_param("si", $kategori, $id_event);
$stmt_related->execute();
$related_events = $stmt_related->get_result();
$stmt_related->close();
...
<body>
    <div class="error-box">
        ...
        <a href="event.php" class="btn-back">← Kembali ke Daftar Events</a>
    </div>
</body>
</html>
<?php exit(); ?>
...
<body>
    ...
    <!-- Back Button -->
    <a href="event.php" class="back-button">
        ← Kembali ke Event
    </a>
    ...
    <section class="related-section">
        <h2 class="related-title">Event Lainnya dalam Kategori <?php echo $kategori; ?></h2>
        <div class="related-grid">
            ...
            while($related = $related_events->fetch_assoc()): 
            ...
            ?>
            <a href="baca_event.php?id=<?php echo $related['id_event']; ?>" class="related-item">
                <div 
                    class="related-image"
                    style="
